...
from utils import generate_timetable, send_timetable_pdf
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if not usn or not email:
                skipped += 1
                continue
            existing_student = db.query(Student).filter((Student.usn == usn) | (Student.email == email)).first()
            if existing_student:
                skipped += 1
                continue
            ...
            uploaded += 1
        db.commit()
        return JSONResponse(content={"message": f"Students uploaded successfully: {uploaded} added, {skipped} skipped."})
...
async def upload_teachers(file: UploadFile = File(...), db: Session = Depends(get_db)):
    try:
        ...
        for _, row in df.iterrows():
            ...
            uploaded += 1
        db.commit()
        return JSONResponse(content={"message": f"Teachers uploaded successfully: {uploaded} added, {skipped} skipped."})
    except Exception as e:
        db.rollback()
        return JSONResponse(status_code=500, content={"error": str(e)})
...
def generate_timetable_endpoint(section: str = Body(...), semester: int = Body(...), db: Session = Depends(get_db)):
    ...
    return {"timetable": enriched_timetable}
...
def get_timetable(section: str, db: Session = Depends(get_db)):
    ...
    for t in timetable:
        ...
        students_in_class = db.query(Student).filter(Student.section == section).all()
        student_usns = [s.usn for s in students_in_class]
        result.append({
            "day": t.day,
            "time": t.time,
            "subject": t.subject,
...
def get_notifications(student_id: int, db: Session = Depends(get_db)):
    ...
    result = [{"message": n.message, "timestamp": n.timestamp} for n in notifications]
    return JSONResponse(content=result)
